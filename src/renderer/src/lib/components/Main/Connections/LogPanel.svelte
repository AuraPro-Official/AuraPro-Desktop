...
>
  ...
  <div class="flex items-center justify-between px-3 py-1 shrink-0 border-b border-white/[0.06]">
    <div class="flex items-center gap-2">
      ...
      {#if onRestart}
        <button
          class="p-1 rounded-md hover:bg-white/[0.08] transition bg-transparent border-none cursor-pointer {restarting
            ? 'opacity-30 pointer-events-none'
            : 'opacity-40 hover:opacity-80'} text-white"
          disabled={restarting}
          aria-label={$i18n.t('common.restart')}
          onclick={restartService}
          use:tooltip={$i18n.t('common.restart')}
        >
          {#if restarting}
            <div
