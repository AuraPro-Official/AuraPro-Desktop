<script lang="ts">
  ...
  onMount(async () => {
    ...
    cleanupDataListener = window.electronAPI.onData((data: UpdateEvent) => {
      switch (data.type) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      updateStatus = 'error'
      updateError = getErrorMessage(error, 'Download failed')
    }
  }
  ...
  const parseChangelog = (md: string) => {
    ...
    // Older installations used plain-text sections such as
    // "AuraPro-3.9.3-2026-07-25". Keep them readable during upgrades.
    if (entries.length === 0) {
  ...
  const plainChangelogText = (value: string): string =>
    value.replace(/\*\*(.+?)\*\*/g, '$1').replace(/`(.+?)`/g, '$1')

  const changelogBodyLines = (
    md: string
      ...
      .map((line) => {
        if (line.startsWith('###')) {
          return {
            ...
            text: plainChangelogText(line.replace(/^###\s*/, ''))
          }
        }
        if (line.startsWith('-')) {
          return {
        ...
        return { kind: 'text' as const, text: plainChangelogText(line) }
      })
  }
</script>
...
<div class="flex flex-col divide-y divide-white/[0.04]">
  ...
  {#if openWebuiVersion}
    ...
    >
      ...
      <div class="text-[12px] opacity-30 group-hover:opacity-50 transition self-center">
        {openWebuiVersion}
      </div>
    </button>
  {/if}
  ...
  {#if sherpaVersion}
    <button
      class="w-full py-4 flex items-center justify-between bg-transparent border-none cursor-pointer group"
      onclick={() => openRelease('k2-fsa/sherpa-onnx', sherpaVersion!, 'v')}
    >
      <div class="text-[13px] opacity-70">Sherpa ONNX</div>
      <div class="text-[12px] opacity-30 group-hover:opacity-50 transition">{sherpaVersion}</div>
    </button>
  {/if}
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        {:else if updateStatus === 'downloaded'}
          <div class="text-[11px] opacity-40 mt-0.5">
            {isMac
              ? text('安装器已下载，请打开完成安装', 'Installer downloaded - open it to finish')
              : $i18n.t('settings.about.updateReady')}
          </div>
  ...
  <div class="py-4">
    ...
    {#if changelogOpen}
      <div class="mt-3 max-h-64 overflow-y-auto pr-1">
        ...
        {:else}
          {#each changelogEntries as entry, i (entry.version)}
            {#if i > 0}
            ...
            <div>
              ...
              {#if entry.body}
                <div class="mt-1">
                  {#each changelogBodyLines(entry.body) as line, index (index)}
                    {#if line.kind === 'heading'}
                      <div class="text-[11px] opacity-50 font-semibold mt-3 mb-1">
                        {line.text}
                      </div>
                    {:else}
                      <div class="text-[11px] opacity-40 pl-2 leading-relaxed">
                        {line.kind === 'item' ? '• ' : ''}{line.text}
                      </div>
                    {/if}
                  {/each}
                </div>
...
<div class="text-[10px] opacity-15 mt-2 leading-relaxed">
  {$i18n.t('settings.about.attribution')}
</div>
