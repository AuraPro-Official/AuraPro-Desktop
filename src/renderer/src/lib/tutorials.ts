...
export const tutorialUiText = {
  intro: {
    'zh-CN':
      '这里按安装、更新和常用功能整理了 AuraPro 教程。桌面端和 WebUI 使用同一套内容，视频链接会直接跳到对应时间线。',
    'zh-TW':
      '這裡按安裝、更新和常用功能整理了 AuraPro 教程。桌面端和 WebUI 使用同一套內容，影片連結會直接跳到對應時間線。',
    'en-US':
      'AuraPro tutorials are grouped by install, updates, and common features. Desktop and WebUI share the same content, and video links jump to the matching timestamp.'
  },
  searchPlaceholder: {
    'zh-CN': '搜索教程、功能或问题',
    'zh-TW': '搜尋教程、功能或問題',
    ...
    items: [
      {
        ...
        steps: [
          text(
            '打开本地 AuraPro 后，可以直接聊天、选择模型、上传文件或使用扩展模式。',
            'After opening local AuraPro, users can chat, select models, upload files, or use extension modes.'
          ),
          text(
      ...
      },
      {
        ...
        steps: [
          ...
          text(
            '局域网共享可以让同一网络内的其他设备访问本机 AuraPro，注意网络和权限安全。',
            'LAN sharing lets other devices on the same network access local AuraPro. Pay attention to network and permission safety.'
          ),
          text(
